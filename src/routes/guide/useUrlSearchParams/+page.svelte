...
<DocTpl title="useUrlSearchParams">
	<div slot="description">
		<p>Offers reactive sync between your Svelte state and the browser’s query parameters.</p>
	...
	<div slot="visual-example">
		...
		<Browser body="p-4 bg-gray-950/50">
			<div>
				<div class="flex">
					<Button on:click={() => params.setParams({ page: "2" })}>Go to Page 2</Button>
					<InputText
						bind:value={$params.filter}
						on:input={() => params.setParams({ filter: $params.filter })}
