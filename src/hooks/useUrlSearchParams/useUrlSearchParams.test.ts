...
describe("Hook - useUrlSearchParams", () => {
	...
	it("reads initial params from search mode", () => {
		const store = useUrlSearchParams("history", {
			initialValue: { a: "0" },
			windowRef: mockWin,
		});
	...
	it("updates params & writes URL", () => {
		...
		expect(mockWin.history.replaceState).toHaveBeenCalledWith({}, "", "/page?a=1&c=3");
	...
	it("popstate triggers read", () => {
		...
		const listener = mockWin.addEventListener.mock.calls.find((c: any[]) => c[0] === "popstate")[1];
